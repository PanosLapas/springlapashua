...
<div class="container" style="padding-left: 10%">
	...
	<div class="alert alert-info">
		<c:if test="${committe.id == 0}" >
	  	...
	  	<c:if test="${committe.id > 0}" >
	  		<strong>Tip!</strong> Τροποποιήστε τα στοιχεία της επιτροπής και διαχειριστείτε τα μέλη που θα την απαρτίζουν.
	  	</c:if>
	</div>
	<br>
	<div class="row-fluid" id="dvContainer">
		<form:form method="POST" action="/committee/committe/create" id="createComForm" name="myform" 
			...
			<div class="col-sm-12">
						  ...
						  <c:when test="${members.size() >= 1}">
						  	<c:forEach items="${members}" var="member">
							   <div class="well" style="margin-bottom:10px; padding-top:8px; padding-bottom:8px;">
							   		<label><c:out value="${member.lastName} ${member.firstName} - " /><i><c:out value="${member.role}" /></i></label>
				...
				<input type="button" class="btn btn-lg btn-danger" onclick="location.href='/committee/committe/all'" value="Ακύρωση"/>
				<br><br><br>
			</div>
		</form:form>
	</div>
			...
			.ready(
					function() {
						...
						$('.DeleteMemberBtnID').on('click', function () {
							var url = '/committee/committe/deleteMember/' + $(this).val();
							window.location = url;
						});
						...
						$('.AddBtnID').on('click', function () {
							...
							var url = '/committee/committe/addMember/' + $(this).val() + '&' + member + '&' + isAdmin; 
							window.location = url;
						});
