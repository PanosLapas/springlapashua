...
<div class="container" style="padding-left: 10%">
						...
						<tr>
							...
							<td class="center-td">
								<c:choose>
								  ...
								  <c:otherwise>
								  		<a href="<c:url value="/committe/downloadAll/${comm.committe.id}" />"><c:out value="${comm.documents.size()} αρχεία" /></a>
								  </c:otherwise>
								</c:choose>
							</td>
							<td class="center-td">
								<c:if test="${comm.committe.isCommitteAdmin == true}" >
									&nbsp;<a href="<c:url value="/committe/modify_bymember/${comm.committe.id}&${comm.committe.isCommitteAdmin}"/>">Επεξεργασία&nbsp;<i
									class="glyphicon glyphicon-pencil"></i></a>&nbsp;
								</c:if> 
								<c:if test="${comm.committe.isCommitteAdmin == false}" >
									&nbsp;<a href="<c:url value="/committe/modify_bymember/${comm.committe.id}&${comm.committe.isCommitteAdmin}"/>">Προβολή&nbsp;<i
	...
	<a href="<c:url value='/committe/download/${doc.id}' />">Download</a> --%>
</body>
<script>
	$(document).ready(function() {
		...
		$('#Clear').on('click', function () {
			var url = '/committee/committe/all_bymember' + $(this).val();
			window.location = url;
		});
	...
	function showCommittes(committes,page)
	{
		...
		for ( var i = 0, len = committes.length; i < len; ++i) 
		{
            ...
            var isCommitteAdmin=comm.committe.isCommitteAdmin;
            
            if(comm.documents.length == 0)
           	{
            	if(isCommitteAdmin == true)
           		{
            		$('#tbodyid').append("<tr><td class='set-height'>" + num +"</td>" +
                 			...
                 			"<td class='center-td set-height'><a href='<c:url value='/committe/modify_bymember/" + comm.committe.id + "&" + isCommitteAdmin + "'/>'>Επεξεργασία &nbsp<i class='glyphicon glyphicon-pencil'></i></a></td>" +
                 			"</tr>"
                 	 );
           		}
            	else
           		{
            		$('#tbodyid').append("<tr><td class='set-height'>" + num +"</td>" +
                 			...
                 			"<td class='center-td set-height'>" + new_date + "</td>" +
                 			"<td class='center-td set-height'>-</td>" +
                 			"<td class='center-td set-height'><a href='<c:url value='/committe/modify_bymember/" + comm.committe.id + "&" + isCommitteAdmin + "'/>'>Προβολή &nbsp<i class='glyphicon glyphicon-eye-open'></i></a></td>" + 
                 			"</tr>"
                 	 );
           		}
           	}
